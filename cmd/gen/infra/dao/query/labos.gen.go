...
func newLabo(db *gorm.DB, opts ...gen.DOOption) labo {
	...
	_labo.Property = laboHasOneProperty{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Property", "model.Property"),
	}
...
type laboHasOneProperty struct {
	db *gorm.DB

	field.RelationField
}

func (a laboHasOneProperty) Where(conds ...field.Expr) *laboHasOneProperty {
	if len(conds) == 0 {
		return &a
	}
...
func (a laboHasOneProperty) WithContext(ctx context.Context) *laboHasOneProperty {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a laboHasOneProperty) Session(session *gorm.Session) *laboHasOneProperty {
	a.db = a.db.Session(session)
	return &a
}

func (a laboHasOneProperty) Model(m *model.Labo) *laboHasOnePropertyTx {
	return &laboHasOnePropertyTx{a.db.Model(m).Association(a.Name())}
}

type laboHasOnePropertyTx struct{ tx *gorm.Association }

func (a laboHasOnePropertyTx) Find() (result *model.Property, err error) {
	return result, a.tx.Find(&result)
}

func (a laboHasOnePropertyTx) Append(values ...*model.Property) (err error) {
...
func (a laboHasOnePropertyTx) Replace(values ...*model.Property) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a laboHasOnePropertyTx) Delete(values ...*model.Property) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a laboHasOnePropertyTx) Clear() error {
	return a.tx.Clear()
}

func (a laboHasOnePropertyTx) Count() int64 {
	return a.tx.Count()
}
