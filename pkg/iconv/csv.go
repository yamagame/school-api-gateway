		...
		for k, v := range record {
			...
			if len(matches) > 0 {
				...
				if _, ok := r[head]; !ok {
					...
					h = r[head].([]map[string]interface{})
				}
				h = append(h, v.(map[string]interface{}))
				r[head] = h
			} else {
				r[k] = v
			}
...
func ReadCSV(in io.Reader) ([]map[string]interface{}, error) {
	// CSVリーダーを作成
		...
		for head, v := range record {
			...
			if len(matches) > 0 {
				...
				if _, ok := item[head]; !ok {
					item[head] = map[string]interface{}{}
				}
				t := item[head].(map[string]interface{})
				t[val] = v
			} else {
				item[head] = v
			}
